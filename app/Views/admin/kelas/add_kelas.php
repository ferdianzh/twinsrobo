                ...
                <form action="/kelas/save" method="post" enctype="multipart/form-data">
                    ...
                    <div class="input-group mb-3">
                        ...
                        <select class="rounded-right custom-select <?= ($validation->hasError('id_mentor')) ? ' is-invalid' : ''; ?>" id="inputGroupSelect01" name="id_mentor">
                            <option value="<?= (old('id_mentor')) ? old('id_mentor') : ''; ?>" selected hidden><?= (old('id_mentor')) ? old('id_mentor') : 'Choose...'; ?></option>
                            <?php
                            foreach ($mentor as $mentor) :
                                $dt_user = $user->where(['id' => $mentor['id_user']])->first();
                            ?>
                                <option value="<?= $mentor['id']; ?>"><?= $mentor['id'] . '. ' . $dt_user['nama_depan'] . ' ' . $dt_user['nama_belakang'];
                                                                        ?></option>
                            <?php
                            endforeach;
                            ?>
                    ...
                    <div class="input-group mb-3">
                        ...
                        <input type="text" class="rounded-right form-control<?= ($validation->hasError('nama_kelas')) ? ' is-invalid' : ''; ?>" placeholder="" aria-label="Username" aria-describedby="basic-addon1" name="nama_kelas" value="<?= old('nama_kelas'); ?>">
                        <div class="invalid-feedback">
